#!/usr/bin/env python3
"""
Enhanced Color-Based Anime Recommendation Demo

This script demonstrates the color palette analysis and visual style recommendation
features in the ani-sage recommendation engine. It allows users to select their
preferred color palettes and visual styles, then provides personalized recommendations
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Enhanced Color-Based Anime Recommendation Demo")
    ...
    parser.add_argument("--no-trailers", action="store_true", help="Disable trailer lookup")
    parser.add_argument("--demo-mode", action="store_true", help="Run in demo mode with predefined data")
    parser.add_argument("--force-api-keys", action="store_true", help="Force using API keys even in demo mode")
    ...
    # Create recommendation engine - use demo mode unless force-api-keys is specified
    use_demo_mode = args.demo_mode and not args.force_api_keys

    # Try to get the engine with or without demo mode
    try:
        engine = get_recommendation_engine(demo_mode=use_demo_mode)
    except Exception as e:
        print(f"{RED}Failed to initialize the recommendation engine: {e}{RESET}")
        print(f"{YELLOW}Falling back to demo mode (some features may be limited){RESET}")
        engine = get_recommendation_engine(demo_mode=True)

    # Create sample anime database
    ...
    options = RecommendationOptions(
        ...
        color_weight=0.5,  # Higher weight for color-based recommendations in this demo
        generate_explanations=True,
        include_trailers=not args.no_trailers,
        demo_mode=args.demo_mode or use_demo_mode  # Pass demo mode to options
    )

    if args.demo_mode:
        print(f"{YELLOW}Running in demo mode - using simulated data{RESET}")
        # In demo mode, we'll always include trailers but with dummy data
        options.include_trailers = True
