// Simple test script to verify MALSync API functionality
// Import fetch for Node.js environment
import fetch from 'node-fetch';

// If running in a CommonJS environment, use this instead:
...
async function testMalSyncApi() {
  ...
  for (const anime of animeList) {
    ...
    try {
      ...
      console.error(`✗ Exception when testing ${anime.title}: ${error.message}`);
    }
  }
}
...
(async () => {
  ...
  console.log('\n\nRecommendations:');
  console.log('1. Implement the manual mapping table as fallback');
  console.log('2. Improve TMDb search logic to add "anime" or "movie" keywords');
  console.log('3. Consider trying Japanese titles for better matches');
})();